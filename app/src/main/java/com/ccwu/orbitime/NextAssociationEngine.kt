...
/** Predicts next Chinese words/short phrases from packaged local N-grams. */
class NextAssociationEngine(private val context: Context) {
    ...
    private val unigramTop: List<TokenScore> by lazy {
        unigram.entries.sortedByDescending { it.value }.take(64)
            .map { TokenScore(it.key, it.value, 0.18) }
    }

    private val bigramNext: Map<String, List<TokenScore>> by lazy {
        buildNextIndex(bigram, 2, 0.72)
    }

    private val trigramNext: Map<String, List<TokenScore>> by lazy {
        buildNextIndex(trigram, 3, 1.08)
    }

    fun suggestions(rawContext: String, limit: Int = 24): List<String> {
        val curated = NextPhraseData.suggestions(rawContext, limit)
        val contextTokens = extractContextTokens(rawContext)
        if (contextTokens.isEmpty()) return curated.take(limit)
    ...
    private fun nextOptions(history: List<String>): List<TokenScore> {
        ...
        if (history.size >= 2) {
            ...
            trigramNext[key].orEmpty().forEach { score ->
                merged[score.token] = score
            }
        }
    ...
    private fun buildNextIndex(source: Map<String, Int>, n: Int, weight: Double): Map<String, List<TokenScore>> {
        ...
        return temp.mapValues { (_, bucket) ->
            bucket.entries.sortedByDescending { it.value }.take(48)
    ...
    private fun loadNGramAsset(n: Int): Map<String, Int> {
        return try {
            ...
            context.assets.open("ime/ngram$n.odict").bufferedReader(Charsets.UTF_8).useLines { lines ->
                lines.forEach { raw ->
    ...
    companion object {
        ...
        private const val MAX_BRANCHES = 8
        private const val BEAM_WIDTH = 18
        private const val MAX_SUGGESTION_CHARS = 24
    }
}
