...
class PetRepository(private val context: Context) {
    ...
    fun profile(): PetProfile {
        ...
        val equippedOutfitId = prefs.getString(KEY_EQUIPPED_OUTFIT_ID, null)
        ...
        return PetProfile(
            ...
            equippedOutfitName = OutfitCatalog.byId(equippedOutfitId)?.name,
    ...
    fun compactStatus(): String {
        ...
        return if (p.displayMode == DISPLAY_HIDDEN) {
            ...
            "${p.petName} · ${p.stageName} · ${p.progressPercent}%"
        }
    }
    ...
    fun outfitCatalogLine(): String {
        val current = profile().equippedOutfitId
    ...
    fun checkIn(): ActionResult {
        ...
        if (last == today) {
            return ActionResult("今日已签到 · 连续 ${profile().checkInStreak} 天", profile())
        }
        ...
        prefs.edit()
            ...
            .apply()
        addExp(5)
        return ActionResult("签到成功 +$starGain Stars · 连续 $newStreak 天", profile())
    }

    fun recordTypedChars(count: Int) {
        if (count <= 0) return
        val today = todayKey()
        val currentToday = if (prefs.getString(KEY_TODAY_TYPED_DATE, null) == today) {
            prefs.getInt(KEY_TODAY_TYPED_CHARS, 0)
        } else {
            0
        }
        ...
        if (expGain > 0) {
            editor.putInt(KEY_EXP, min(prefs.getInt(KEY_EXP, 0) + expGain, MAX_EXP))
        }
        editor.apply()
    }
    ...
    fun adoptRandom(): ActionResult {
        ...
        if (unowned.isEmpty()) {
            return ActionResult("当前可领养宠物已收集完，可切换已有宠物", profile())
        }
        val today = todayKey()
        ...
        if (stars < cost) {
            return ActionResult("Stars 不足：需要 $cost，当前 $stars；明天可免费开蛋一次", profile())
        }
        ...
        if (dailyFree) {
            editor.putString(KEY_LAST_ADOPT_DATE, today)
        } else {
            editor.putInt(KEY_STARS, stars - cost)
        }
        editor.apply()
    ...
    fun equipOutfit(outfitId: String?): ActionResult {
        ...
        if (outfit?.isPro == true && !ProGate.isProUnlocked(context)) {
            return ActionResult("这是 Pro 装扮占位，当前不可用", profile())
        }
    ...
    fun localChatLine(): String {
        val p = profile()
        return when {
            p.displayMode == DISPLAY_HIDDEN -> "我在隐身，不会打扰你。"
            p.nextStageExp != null && p.nextStageExp - p.exp <= 10 -> "快进化了，还差 ${p.nextStageExp - p.exp} EXP。"
            p.todayTypedChars >= 1000 -> "今天输入很多，节奏不错。"
            p.checkInStreak >= 7 -> "连续 ${p.checkInStreak} 天了，保持住。"
            p.stage >= 4 -> "我已经成熟，会安静陪你输入。"
            p.stage == 3 -> "轮廓稳定了，继续积累文字。"
            p.stage == 2 -> "我正在成长，候选上屏也会增加经验。"
            else -> "我还在孵化，打字会让我成长。"
        }
    }
    ...
    private fun stageNameFor(exp: Int): String = when (stageFor(exp)) {
        1 -> "Seed"
        2 -> "Junior"
        3 -> "Teen"
        else -> "Adult"
    ...
    private fun nextStageExp(exp: Int): Int? = when {
        exp < 50 -> 50
        exp < 200 -> 200
        exp < 500 -> 500
        else -> null
