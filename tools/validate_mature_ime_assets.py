...
def main() -> int:
    ...
    min_cn = int(policy["minimum_aosp_entries"])
    min_en = int(policy["minimum_english_entries"])
    require(int(stats.get("aosp_lexicon_entries", 0)) >= min_cn, f"AOSP lexicon too small: {stats.get('aosp_lexicon_entries')}")
    require(int(stats.get("esdb_english_entries", 0)) >= min_en, f"ESDB English pack too small: {stats.get('esdb_english_entries')}")
    require(int(counts.get("lexicon", 0)) >= min_cn, f"runtime lexicon too small: {counts.get('lexicon')}")
    require(int(counts.get("english", 0)) >= min_en, f"runtime English pack too small: {counts.get('english')}")

    ngrams = counts.get("ngrams", {})
    require(int(ngrams.get("1", 0)) >= 1000, f"1-gram pack too small: {ngrams.get('1')}")
    require(int(ngrams.get("2", 0)) >= 5000, f"2-gram pack too small: {ngrams.get('2')}")
    require(int(ngrams.get("3", 0)) >= 5000, f"3-gram pack too small: {ngrams.get('3')}")
    ...
    for key in ("aosp_pinyin", "aosp_notice", "esdb_en_us", "esdb_copyright"):
    ...
    require('versionCode = 15' in gradle, "versionCode is not 15")
    require('versionName = "0.15.0"' in gradle, "versionName is not 0.15.0")
